...
select bidder_id,count(bidder_id) as total_matches,
sum(if(bid_status='Won',1,0)) as matches_won,((sum(if(bid_status='Won',1,0))/count(bidder_id))*100) as Percentage_of_wins from ipl_bidding_details
...
select st.stadium_name,st.city,count(ims.stadium_id) as No_of_matches from ipl_match_schedule ims
right join ipl_stadium st on ims.stadium_id=st.stadium_id
where ims.status!='cancelled'
...
select st.stadium_id,st.stadium_name,st.city,ims.match_id,ims.status, 
count(st.stadium_id) as no_of_matches,sum(if(im.toss_winner=im.match_winner,1,0)) as no_of_teams_win_toss_and_match,
(sum(if(im.toss_winner=im.match_winner,1,0))/count(st.stadium_id))*100 as percentage_of_teams_win_toss_and_match from ipl_stadium st
join ipl_match_schedule ims on st.stadium_id=ims.stadium_id
...
select ibd.bidder_id,group_concat(ibd.bid_team) as group_of_bid_teams,
group_concat(ibd.bid_date) as group_of_bid_dates,group_concat(it.team_name) as group_of_bid_team_names,
count(ibd.bidder_id) as total_bids 
...
select match_id, if(match_winner=1,team_id1,team_id2) as Team_Id, win_details from ipl_match ;
...
select its.TEAM_ID,it.TEAM_NAME,its.TOURNMT_ID,sum(its.MATCHES_PLAYED) as Total_Matches_Played,sum(its.MATCHES_WON) as Total_Matches_Won,
sum(its.MATCHES_LOST) as Total_Matches_Lost from ipl_team_standings its
...
select it.Team_Name,ip.Player_Name,itp.Player_Role from ipl_team_players itp
...
select itp.Team_ID,it.Team_Name,group_concat(ip.Player_name) as Names_of_All_Rounders,count(itp.team_id) as No_of_All_Rounders from ipl_team_players itp
join ipl_team it on itp.team_id=it.team_id
join ipl_player ip on itp.player_id=ip.player_id
